...
Strength SMP Troll Items will support Paper and Purpur only on Minecraft 26.1,
26.1.1, 26.1.2, and 26.2. It will compile against Paper API and target one
binary-compatible jar across all eight server/version combinations. Purpur is
supported through its Paper compatibility. Spigot and CraftBukkit are not
supported or tested, although inherited `org.bukkit` types and `plugin.yml`
remain necessary parts of the Paper plugin API.
...
rejected because they would be fragile across four Minecraft versions and
undermine the single-jar goal. Keeping ProtocolLib as either a hard or optional
...
final damage. The same jar must compile against the oldest and newest supported
Paper APIs. A live Paper/Purpur matrix must verify damage timing and three-player
...
project version will receive a MAJOR bump only after explicit confirmation, as
required by the repository release rules.
